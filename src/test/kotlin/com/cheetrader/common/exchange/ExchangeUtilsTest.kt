...
class ExchangeUtilsTest {
    @Test
    fun `extractTrailingParams returns null when not active`() {
        assertNull(extractTrailingParams(mapOf("trailingActive" to "false")))
    }
    ...
    fun `extractTrailingParams returns null when deviation is zero`() {
        assertNull(extractTrailingParams(mapOf("trailingActive" to "true", "trailingDeviation" to "0.0")))
    }

    @Test
    fun `extractTrailingParams returns null when deviation is negative`() {
        assertNull(extractTrailingParams(mapOf("trailingActive" to "true", "trailingDeviation" to "-0.01")))
    }
